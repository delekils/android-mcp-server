...
class Rule:
    ...
    def from_definition(cls, definition: RuleDefinition) -> "Rule":
        ...
        return cls(
            ...
            evidence=[EvidenceWeight.model_validate(item) for item in definition.evidence],
            regex=regex,
        )
...
def _load_rule_file(path: Path) -> list[Rule]:
    ...
    if isinstance(raw, dict) and "rules" in raw:
        ...
        rules_raw = raw
    return [Rule.from_definition(RuleDefinition.model_validate(item)) for item in rules_raw]
